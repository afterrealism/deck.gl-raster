...
import type { TileMatrix, TileMatrixSet } from "./raster-tileset/types.js";
...
> extends CompositeLayerProps {
  ...
  /**
   * Maximum number of new tile requests to start per frame
   * @default 10
   */
   ...
   * Higher values prevent tiles from disappearing during zoom transitions
   * Recommended: 500+ for smooth zoom across many levels
   * @default 500
   */
  maxCacheSize?: number;
   ...
   * Higher values keep more tiles in memory for smoother zoom transitions
   * Recommended: 500MB+ for desktop applications
   ...
   * - 'best-available': Show parent tiles while children are loading (recommended)
   * - 'no-overlap': Only show tiles at current zoom level
   * - 'never': Never show parent tiles
...
const defaultProps: Partial<COGLayerProps> = {
  ...
  maxCacheSize: 500, // Large cache prevents tiles from disappearing during zoom
  maxCacheByteSize: 500 * 1024 * 1024, // 500MB keeps tiles in memory across zoom levels
...
> extends CompositeLayer<COGLayerProps<DataT>> {
  ...
  async _parseGeoTIFF(): Promise<void> {
    ...
    // Since source is always EPSG:3857
    const converter = createConverter("EPSG:3857", "EPSG:4326");
  ...
  renderTileLayer(
    ...
    return new TileLayer<GetTileDataResult<DataT>>({
      ...
      maxRequests: this.props.maxConcurrentRequests,
